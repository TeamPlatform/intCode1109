...
public class Defect_Tracking_35 extends LaunchBrowser {
	...
	public void user_create_the_subtask_with_select_Lov_consists_containing_only_numbers() throws Throwable {
		try
		{
			...
			bp.waitForElement();
			bp.waitForElement();
			actual[206]=ep.searchDefect(defectId);
			bp.waitForElement();
			actual[207]=ep.clickOnSubTask();
			bp.waitForElement();
			actual[208]=dt.createDefectSubtask(issueType);
			actual[209]=dt.fileNewDefect(summary, components, descType, desc, fixVersion, priority, assignee, labels, sprint, env, epicLink, affectsVersion, linkedIssue);
			bp.waitForElement();

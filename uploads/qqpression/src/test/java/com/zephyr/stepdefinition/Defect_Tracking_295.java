...
public class Defect_Tracking_295 extends LaunchBrowser {
    ...
    String fileName="Defect_Tracking_295";
	...
	public void adds_role_with_defect_admin_permission() throws Throwable {
		try
		{
			...
			{
				bp.waitForElement();
				actual[5]=ccp.addCustomRole(name, description, administrationApps, projectApps);
				bp.waitForElement();
				ccp.closeCustomizationTab.click();
			}		
		}
	...
	public void user_goes_to_User_setup_and_adds_a_user_with_above_created_role() throws Throwable {
		try
		{
			String name=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_TPE,"Custom_Username");
			...
			boolean result=ccp.checkAvailableCustomRole(name);
			String userName=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_TPE,"Custom_Username_1");
			String value[]=userName.split("\\.");
			String firstName=value[0];
			String lasTName=value[1];
			String projectName[]=new String[1];
			projectName[0]=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_TPE,"Normal_Project_1");
			if(result==true)
			{
			bp.waitForElement();
			actual[6]=ccp.moveToCreateCustomUser();
			actual[7]=ccp.createUser(firstName, lasTName, type, role, email, title, location);
			actual[8]=pp.selectUser(name);
			actual[9]=pp.assignProject(projectName);
			}
			else
			{
	...
	public void should_be_able_to_add_created_role_to_user() throws Throwable {
		try
		{
			...
			String uname=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_TPE,"Custom_Username_1");
			String password=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_TPE,"Custom_Password_1");
			bp.waitForElement();
			bp.waitForElement();

...
public class Defect_Tracking_70 extends LaunchBrowser{
	...
	DefectTracking dt;
	String fileName="Defect_Tracking_70";
	...
	try
    {
	     ...
	     String managerUname=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_TPE,"Manager_Username_1");
	     String managerPass=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_TPE,"Manager_Password_1");
	     actual[0]=lp.enterUname(managerUname);
	     actual[1]=lp.enterPass(managerPass);
	...
	try
	   {
		 ...
		 String leadUname=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_TPE,"Lead_Username_1");
	     String leadPass=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_TPE,"Lead_Password_1");
	     actual[7]=lp.enterUname(leadUname);
	     actual[8]=lp.enterPass(leadPass);
	...
	try
	   {
		 String Uname=Excel_Lib.getData(INPUT_PATH_3,"Defect Tracking",1,64); 
		 String Pwd=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_TPE,"Jira_Password");
		 actual[12]=req.dtsLoginDetail(Uname,Pwd,Pwd);
	   }
	   catch(Exception e)
		{
		   lb=new LaunchBrowser();
	...
	try
	{
	     ...
	     bp.waitForElement();
	     actual[14]=ejp.loginToExternalJira(Uname,Pwd);
	}
	catch(Exception e)
	{
	   lb=new LaunchBrowser();
	...
	try
	   {
		 ...
		 actual[16]=ejp.enterAdministratorAccessPassword(Pwd);
	   }
	   catch(Exception e)
		{
		   lb=new LaunchBrowser();
	...
	try
	   {
		 String newName=UNIQUE+Excel_Lib.getData(INPUT_PATH_3, "Defect Tracking",1,64);
		 String uName=Excel_Lib.getData(INPUT_PATH_3, "Defect Tracking",1,64);
	...
	try
	   {
		 String uName=UNIQUE+Excel_Lib.getData(INPUT_PATH_3, "Defect Tracking",1,64);
	...
	try
	   {
	     ...
	     actual[21]=lp.enterUname(Uname);
	     actual[22]=lp.enterPass(Pass);
	...
	try
	{
		 ...
		 String Pwd=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_TPE,"Jira_Password");
		 actual[28]=req.dtsLoginDetail(Uname,Pwd,Pwd);
	...
	try
	{
		 ...
		 //pp.clickOnLogout(); 
	     String jira_Url=Property_Lib.getPropertyValue(CONFIG_PATH+CONFIG_FILE_TPE,"JIRA_URL");
	     String newUName=UNIQUE+Excel_Lib.getData(INPUT_PATH_3, "Defect Tracking",1,64);
	     String uName=Excel_Lib.getData(INPUT_PATH_3, "Defect Tracking",1,64);
	     String fullName=Excel_Lib.getData(INPUT_PATH_3, "Defect Tracking",1,66);
		 String emailId=Excel_Lib.getData(INPUT_PATH_3, "Defect Tracking",1,65);
		 String issueType=Excel_Lib.getData(INPUT_PATH_3, "Defect Tracking",2,40);
